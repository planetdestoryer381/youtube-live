...
# 🔑 THE FIX
# =========================
# 1. Double check this key in YouTube Studio!
export YT_STREAM_KEY="u0d7-eetf-a97p-uer8-18ju"

# 2. Use RTMP instead of RTMPS (GitHub handles RTMP better)
YOUTUBE_URL="rtmp://a.rtmp.youtube.com/live2/${YT_STREAM_KEY}"

export FPS=30 # Lowered to 30 for maximum stability/compatibility
export W=1080
export H=1920
...
function init(){
  ents=countries.map(c=>({
    n:c.name, i:c.iso2.toLowerCase(), x:CX+(Math.random()-0.5)*400, y:CY+(Math.random()-0.5)*400,
    vx:(Math.random()-0.5)*400, vy:(Math.random()-0.5)*400, a:true
  }));
  state="PLAY";
}
...
function loop(){
  ...
  if(state==="PLAY"){
    ...
    drawT(`ALIVE: ${alive.length}`, 50, 100, 4, [255,255,255]);
    drawT("TYPE TO ENTER!", 50, 1800, 3, [255,255,0]);
    ...
    ents.forEach((e,idx)=>{
      ...
      for(let j=idx+1;j<ents.length;j++){
        ...
        if(d<R*2){
          let nx=dx/d, ny=dy/d, p=(e.vx*nx+e.vy*ny-(b.vx*nx+b.vy*ny));
    ...
    drawT("WINNER!", CX-150, CY-300, 6, [255,255,0]);
    drawT(winner, CX-(winner.length*15), CY+200, 4, [255,255,255]);
...
while true; do
  ...
  echo "Stream disconnected. Key check: ${YT_STREAM_KEY:0:4}..."
  sleep 5
done
