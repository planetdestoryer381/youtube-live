...
# --- VERIFIED ASSET DOWNLOADER ---
download_flag () {
  local iso=$(echo "$1" | tr 'A-Z' 'a-z')
  [ -s "$FLAGS_DIR/${iso}_60.rgb" ] && return 0
  local png="$FLAGS_DIR/${iso}.png"
  if curl --retry 5 -m 15 -fsSL "https://flagcdn.com/w160/${iso}.png" -o "$png"; then
    # Scaled to 60 for "Normal" size in a tiny arena
    ffmpeg -loglevel error -y -i "$png" -vf "scale=60:60" -f rawvideo -pix_fmt rgb24 "$FLAGS_DIR/${iso}_60.rgb" || true
...
const R=24;          // "Normal" ball size (smaller than 36)
const RING_R=125;    // 2x smaller than 250 (Very compact)
const CX=W/2,CY=700; 
...
// MODIFIED BLIT: No circular clipping, shows entire flag square
function blit(cx,cy,iso,sz){
...
function init(){ents=countries.sort(()=>0.5-Math.random()).map(c=>({n:c.name,i:c.iso2.toLowerCase(),x:CX,y:CY,vx:(Math.random()-0.5)*800,vy:(Math.random()-0.5)*800,f:false}));deadStack=[];state="PLAY";}
function drawUI(){
  ...
  l.forEach(([n,w],i)=>{drawT(`${i+1}. ${n.substring(0,16)}`,60,245+i*35,2,[220,220,220]);drawT(w.toString(),950,245+i*35,2,[255,255,255]);});
  for(let y=1100;y<1920;y++)for(let x=0;x<W;x++){const idx=(y*W+x)*3;rgb[idx]=10;rgb[idx+1]=10;rgb[idx+2]=15;}
  deadStack.forEach((e,idx)=>{const col=idx%20,row=Math.floor(idx/20);blit(45+col*52,1150+row*35,e.i,30);});
}
function loop(){
  for(let i=0;i<rgb.length;i+=3){rgb[i]=165;rgb[i+1]=110;rgb[i+2]=85;}
  const hDeg=(Date.now()/1000*1.5*60)%360;drawUI();
  if(state==="PLAY"){
    for(let a=0;a<360;a+=0.5){let diff=Math.abs(((a-hDeg+180)%360)-180);if(diff<22)continue;const r=a*Math.PI/180;for(let t=-8;t<8;t++){const px=Math.floor(CX+(RING_R+t)*Math.cos(r)),py=Math.floor(CY+(RING_R+t)*Math.sin(r));if(px>=0&&px<W&&py>=0&&py<H){const idx=(py*W+px)*3;rgb[idx]=255;rgb[idx+1]=255;rgb[idx+2]=255;}}}
    ents.forEach((e,i)=>{if(e.f){const ti=deadStack.indexOf(e);const tx=45+(ti%20)*52,ty=1150+Math.floor(ti/20)*35;e.x+=(tx-e.x)*0.2;e.y+=(ty-e.y)*0.2;blit(e.x,e.y,e.i,30);return;}
    for(let j=i+1;j<ents.length;j++){let b=ents[j];if(b.f)continue;let dx=b.x-e.x,dy=b.y-e.y,d=Math.sqrt(dx*dx+dy*dy);if(d<R*2&&d>0){let nx=dx/d,ny=dy/d,ov=(R*2-d)+1;e.x-=nx*(ov/2);e.y-=ny*(ov/2);b.x+=nx*(ov/2);b.y+=ny*(ov/2);let p=(e.vx*nx+e.vy*ny-(b.vx*nx+b.vy*ny));e.vx-=p*nx;e.vy-=p*ny;b.vx+=p*nx;b.vy+=p*ny;}}
    e.x+=e.vx*DT;e.y+=e.vy*DT;let dx=e.x-CX,dy=e.y-CY,dist=Math.sqrt(dx*dx+dy*dy);if(dist>RING_R-R){const ang=(Math.atan2(dy,dx)*180/Math.PI+360)%360;if(Math.abs(((ang-hDeg+180)%360)-180)<22){e.f=true;deadStack.push(e);}else{let nx=dx/dist,ny=dy/dist,dot=e.vx*nx+e.vy*ny;e.vx=(e.vx-2*dot*nx)*1.02;e.vy=(e.vy-2*dot*ny)*1.02;e.x=CX+nx*(RING_R-R);e.y=CY+ny*(RING_R-R);}}blit(e.x,e.y,e.i,60);});
    ...
    drawT("WINNER!",430,500,6,[255,255,255]);blit(W/2,730,winner.i,240);drawT(winner.n,350,870,3,[255,255,255]);if(++timer>300)init();
