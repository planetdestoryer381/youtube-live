...
download_flag () {
  ...
  # Pre-convert to raw RGB once to save CPU during stream
...
echo "--- Optimizing Assets for 60FPS ---"
ISO_LIST=$(grep -oE '"iso2"[[:space:]]*:[[:space:]]*"[^"]+"' "./countries.json" | cut -d'"' -f4 | tr 'A-Z' 'a-z' | sort -u)
for iso in $ISO_LIST; do download_flag "$iso"; done

# --- Node.js Engine (Performance Tuned) ---
cat > /tmp/yt_sim.js <<'JS'
const fs = require('fs');
const W=1080, H=1920, FPS=60, R=35, RING_R=420, DT=1/60, GRAVITY=1200;
const CX=W/2, CY=H/2, FLAGS_DIR="/tmp/flags";
const rgb = Buffer.alloc(W * H * 3);

// Cache common colors to avoid array creation
const BG_COLOR = [158, 100, 75];
const UI_COLOR = [25, 25, 30];
const SLOT_COLOR = [45, 45, 55];
...
function drawRect(x1,y1,w,h,c){
  for(let y=y1;y<y1+h;y++) for(let x=x1;x<x1+w;x++) setP(x,y,c[0],c[1],c[2]);
}
...
function drawT(t,x,y,s,c){
  let cx=x; for(let char of (t||"").toString().toUpperCase()){
    ...
    for(let r=0;r<7;r++) for(let col=0;col<5;col++) if(rows[r]&(1<<(4-col))) for(let sy=0;sy<s;sy++) for(let sx=0;sx<s;sx++) setP(cx+col*s+sx,y+r*s+sy,c[0],c[1],c[2]);
    cx+=6*s;
  }
}

// Memory-cached flags to avoid disk I/O lag
const flagCache = {};
function blit(cx,cy,rad,iso,sz){
  const key = `${iso}_${sz}`;
  if(!flagCache[key]) {
    try { flagCache[key] = fs.readFileSync(`${FLAGS_DIR}/${iso}_${sz}.rgb`); } catch(e){ return; }
  }
  const b = flagCache[key];
...
function init(){
  ents=countries.sort(()=>0.5-Math.random()).slice(0,30).map(c=>({
    ...
    vx:(Math.random()-0.5)*800, vy:(Math.random()-0.5)*800, a:true, falling:false
  }));
  state="PLAY";
}

function drawUI() {
  drawRect(40, 60, 1000, 140, UI_COLOR);
  drawRect(60, 75, 300, 110, SLOT_COLOR);
  ...
  drawRect(390, 75, 300, 110, SLOT_COLOR);
  drawT("Mode", 480, 90, 1, [180, 180, 180]);
  drawT("Last one wins", 410, 115, 2, [255, 255, 255]);
  drawRect(720, 75, 300, 110, SLOT_COLOR);
  drawT("!67 = BAN", 760, 105, 4, [255, 50, 50]);
  drawRect(0, 1450, 1080, 470, [15, 15, 20]);
  // Drawing only first 60 flags in staging to save FPS
  for(let i=0; i<60; i++) {
    let c = countries[i];
    let r = Math.floor(i/15), col = i%15;
    blit(55 + col*70, 1500 + r*65, 18, c.iso2.toLowerCase(), 40);
  }
}
...
function loop(){
  ...
  if(state==="PLAY"){
    ...
    ents.forEach((e, i) => {
      ...
      if(dist > RING_R-R){
        ...
        if(Math.abs(((ang-hDeg+180)%360)-180) < 25){
          e.falling = true; e.vx *= 0.2; e.vy = 200;
        } else {
    ...
    drawT(winner.n.substring(0,15), 580, 920, 3, [255, 255, 255]);
    if(++timer > 300) init();
  }
...
# --- FFmpeg 60FPS High-Stability Stream ---
while true; do
  node /tmp/yt_sim.js | ffmpeg -hide_banner -loglevel error -y \
    -thread_queue_size 1024 -f rawvideo -pixel_format rgb24 -video_size 1080x1920 -framerate 60 -i - \
    -f lavfi -i "anullsrc=channel_layout=stereo:sample_rate=44100" \
    -c:v libx264 -preset ultrafast -tune zerolatency -pix_fmt yuv420p \
    -g 120 -b:v 4500k -maxrate 4500k -bufsize 9000k -f flv "$YOUTUBE_URL"
  sleep 2
done
