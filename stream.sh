...
download_flag () {
  ...
  if curl --retry 5 -m 15 -fsSL "https://flagcdn.com/w160/${iso}.png" -o "$png"; then
    # Smaller assets for the smaller arena
...
const R=16;          // 1. Smaller ball size
const RING_R=125;    // 2. 2x Smaller arena (from 250)
const CX=W/2,CY=700; 
...
// Corrected Blit: Smaller size + Circular Clipping (No squares)
function blit(cx,cy,iso,sz,clipRad){
  ...
  for(let y=0;y<sz;y++)for(let x=0;x<sz;x++){
    if((x-sz/2)**2+(y-sz/2)**2 > clipRad**2) continue; // Forces a circle shape
...
function loop(){
  ...
  if(state==="PLAY"){
    ...
    e.x+=e.vx*DT;e.y+=e.vy*DT;let dx=e.x-CX,dy=e.y-CY,dist=Math.sqrt(dx*dx+dy*dy);if(dist>RING_R-R){const ang=(Math.atan2(dy,dx)*180/Math.PI+360)%360;if(Math.abs(((ang-hDeg+180)%360)-180)<22){e.f=true;deadStack.push(e);}else{let nx=dx/dist,ny=dy/dist,dot=e.vx*nx+e.vy*ny;e.vx=(e.vx-2*dot*nx)*1.02;e.vy=(e.vy-2*dot*ny)*1.02;e.x=CX+nx*(RING_R-R);e.y=CY+ny*(RING_R-R);}}blit(e.x,e.y,e.i,40,R);});
    ...
    drawT("WINNER!",430,500,6,[255,255,255]);blit(W/2,730,winner.i,240,110);drawT(winner.n,350,870,3,[255,255,255]);if(++timer>300)init();
...
# --- STREAM FIXER & START ---
while true; do
  echo "--- Resetting Encoder for Healthy Ingest ---"
