...
# Run from script directory so countries.json is found
...
export W=1080
export H=1920
export FLAGS_DIR="$SCRIPT_DIR/flags"
mkdir -p "$FLAGS_DIR"
...
const fs=require('fs'),path=require('path');const W=1080,H=1920,FPS=60,DT=1/60;
...
const flagCache={};function blit(cx,cy,iso,sz,clipRad){const k=`${iso}_${sz}`;if(!flagCache[k])try{flagCache[k]=fs.readFileSync(path.join(FLAGS_DIR,`${iso}_${sz}.rgb`))}catch(e){return};const b=flagCache[k];if(!b)return;const x0=Math.floor(cx-sz/2),y0=Math.floor(cy-sz/2);for(let y=0;y<sz;y++)for(let x=0;x<sz;x++){if((x-sz/2)**2+(y-sz/2)**2 > clipRad**2) continue;const si=(y*sz+x)*3,di=((y0+y)*W+(x0+x))*3;if(di>=0&&di<rgb.length-3){rgb[di]=b[si];rgb[di+1]=b[si+1];rgb[di+2]=b[si+2];}}}

let ents=[],deadStack=[],winStats={},state="PLAY",timer=0,lastWin="NONE",winner=null;
const countries=JSON.parse(fs.readFileSync(path.join(process.cwd(),'countries.json'),'utf8'));
